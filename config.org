...
(use-package lsp-mode
  ...
  :hook ((lsp-mode . lsp-enable-which-key-integration)
         (lsp-mode . my/lsp-js-ts-indent-setup)
...
#+begin_src emacs-lisp
(defun my/load-project-env-from-shell ()
  "Load env variables from direnv+fish shell for the current project."
  (interactive)
  (let* ((default-directory (or (when (fboundp 'project-root)
                                  (project-root (project-current)))
                                default-directory))
         (env-output (with-temp-buffer
                       (when (zerop
                              (call-process
          ...
          (when (string= var "PATH")
            (setq exec-path
                  (append (parse-colon-path val)
                          (list exec-directory)))))))
    (message "[env] Loaded environment for %s" default-directory)))

(add-hook 'projectile-after-switch-project-hook #'my/load-project-env-from-shell)

(defun my/maybe-load-env-and-start-lsp ()
  (when (derived-mode-p 'typescript-ts-mode 'tsx-ts-mode 'js-ts-mode)
    (my/load-project-env-from-shell)
    (when (executable-find "node")
      (message "[env+lsp] Found node, starting LSP.")
      (lsp-deferred))))

(add-hook 'typescript-ts-mode-hook #'my/maybe-load-env-and-start-lsp)
(add-hook 'tsx-ts-mode-hook #'my/maybe-load-env-and-start-lsp)
(add-hook 'js-ts-mode-hook #'my/maybe-load-env-and-start-lsp)
#+end_src
