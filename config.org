...
(defun my/load-direnv-env ()
  "Load environment using direnv and fish for current buffer's project."
...
(defun my/setup-js-env-and-lsp ()
  "Load direnv environment and start LSP if node is found."
  (my/load-direnv-env)
  (if (executable-find "node")
      (progn
        (message "[env+lsp] Found node, starting LSP.")
        (lsp-deferred))
    (message "[env+lsp] Node not found in PATH after env load.")))

(dolist (hook '(typescript-ts-mode-hook
                tsx-ts-mode-hook
                js-ts-mode-hook))
  (add-hook hook #'my/setup-js-env-and-lsp))
#+end_src
