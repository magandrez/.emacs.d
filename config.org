...
(custom-set-faces
 ...
 '(highlight ((t (:distant-foreground unspecified))))
 '(lazy-highlight ((t (:background "DarkGoldenrod1"
  ...
  (defun setup-install-grammars ()
    ...
    (dolist (grammar
             '((css . ("https://github.com/tree-sitter/tree-sitter-css" "v0.25.0"))
               (elixir . ("https://github.com/elixir-lang/tree-sitter-elixir" "v0.3.5" "src"))
...
SQL formatter from Purcell FTW
#+begin_src emacs-lisp
(use-package sqlformat
  :ensure t
  :config (setq sqlformat-command 'pgformatter
                sqlformat-args '("--no-space-function"))
...
(use-package terraform-mode
  ...
  :mode (("\\.tf\\'" . terraform-mode)
         ...
         ("\\.tfstate\\'" . terraform-mode))
  :config (setq lsp-terraform-ls-enable-show-reference t ;; Code lens is experimental in Terraform mode
...
(use-package sh-script
  :ensure nil
  :config
  (defun my/insert-shell-heading ()
    "Insert custom shell script heading."
    (interactive)
    (goto-line 1)
    (insert "#!/usr/bin/env bash\n")
    (insert "#\n")
    (insert "# Author: Manuel González <manuel@manuel.is> -  DD.MM.YYYY\n")
    (insert "#\n")
    (insert "# Requirements\n")
    (insert "# - \n")
    (insert "# Description\n")
...
(use-package tab-bar
  ...
  (defun my/tab-bar-tab-name ()
    (let ((name (alist-get 'name (tab-bar--current-tab))))
      (propertize (format " 📁 %s " name) 'face 'my/tab-name-face)))
...
For credentials, use `auth-source-pass`, included in Emacs 26.
...
Use git-link to open links to Github (or other hosting) in the browser
from Magit
...
From [[https://mbork.pl/2026-02-02_git-link][Marcin Borkowski]]:
...
(defun my/flycheck-focus-error-list ()
  "Focus the Flycheck errors buffer if it's displayed."
  (let ((buffer (get-buffer "*Flycheck errors*")))
    (when buffer
      (let ((window (get-buffer-window buffer)))
        (when window
          (select-window window))))))

(advice-add 'flycheck-list-errors :after #'my/flycheck-focus-error-list)
...
(use-package move-text
  :ensure t
  :bind (("s-k" . move-text-region-down)
         ("s-i" . move-text-region-up)))
#+end_src
...
(setq sql-mysql-options '("-A" "--default-character-set=utf8")
      org-table-convert-region-max-lines '3001 ;; Useful in org-tables generated by SQL blocks to display large sets of results
...
#+begin_src emacs-lisp
(add-to-list 'load-path "/usr/share/emacs/site-lisp/elpa-src/mu4e-1.14.2")
(require 'mu4e)
(use-package mu4e
  :ensure nil
  :config
  ...
  (setq mu4e-mu-binary "/usr/bin/mu"
        ...
        mu4e-maildir "~/.mail/manuel@manuel.is"
        ...
        message-citation-line-format "On %a, %d %b %Y at %H:%M, %f wrote:\n"
        ...
        (list
         (make-mu4e-context
          ...
          :match-func (lambda (msg)
                        (when msg
                          (or (mu4e-message-contact-field-matches msg :to "manuel@manuel.is")
                              ...
                              (string-match-p "^/manuel" (mu4e-message-field msg :maildir)))))
          :vars `((user-mail-address . "manuel@manuel.is")
                  ...
                  (mu4e-maildir-shortcuts . ((:maildir "/INBOX" :name "Inbox" :key ?i)
                                             (:maildir "/Archive" :name "Archive" :key ?a)
                                             (:maildir "/Drafts" :name "Drafts" :key ?d)
                                             (:maildir "/Trash" :name "Trash" :key ?t)
                                             (:maildir "/Starred" :name "Starred" :key ?S)
                                             (:maildir "/Sent" :name "Sent" :key ?s)))
...
(defun gnus-dired-mail-buffers ()
  ...
  (dolist (buf (buffer-list t) buffers)
...
(global-set-key (kbd "ESC ESC c") #'open-config)
#+end_src
...
#+begin_src emacs-lisp
(global-set-key (kbd "s-d") #'projectile-find-dir)       ;; Find folder in project
(global-set-key (kbd "s-f") #'projectile-find-file)      ;; Find file in project
(global-set-key (kbd "s-p") #'projectile-switch-project) ;; Switch project
(global-set-key (kbd "s-m") #'magit-status)              ;; Magit status
#+end_src
...
#+begin_src emacs-lisp
(defun js-stacktrace-format ()
  "Format a JavaScript stack trace by breaking \\n."
  (interactive)
  (save-excursion
    (goto-char (point-min))
...
#+begin_src emacs-lisp
(defun my/compile ()
  "Save buffer and always prompt for compile command, remembering last one."
  (interactive)
  (save-buffer)
  (let ((display-buffer-alist
         '(("\\*compilation\\*"
            (display-buffer-same-window)))))
    (call-interactively #'compile '(4))))

(defvar my/run-command nil)

(defun my/run ()
  "Save buffer and run a command, remembering last run command."
  (interactive)
  (save-buffer)
  (let* ((cmd (read-shell-command "Run: " my/run-command))
         (my/run-command cmd)
...
(global-set-key (kbd "C-c C-r") #'my/run)
#+end_src
