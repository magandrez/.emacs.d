...
*** JSON
--> Handled with Treesitter and LSP.
*** Dot
...
#+begin_src emacs-lisp
(defun my/lsp-js-ts-indent-setup ()
  "Set indentation level to 2 for TS/JS tree-sitter modes with LSP."
  (when (derived-mode-p 'typescript-ts-mode 'tsx-ts-mode 'js-ts-mode 'javascript-mode)
...
(use-package prettier
  :ensure t
  :hook ((typescript-ts-mode tsx-ts-mode js-ts-mode js-mode) . prettier-mode))
#+end_src
...
(use-package lsp-mode
  ...
  :diminish "LSP"
  :custom
    (lsp-disabled-clients '(vue-semantic-server angular-ls deno tfls credo-language-server)) ;; For Terraform, don't use tfls, but use Hashicorp's official implementation
...
(use-package company
  :ensure t
  :init
  (setq company-backends '(company-files company-keywords company-ispell company-dabbrev-code company-etags))
  (setq-default
     ...
     company-selection-wrap-around t) ;; Wrap around to beginning when you hit bottom of
...
**** Load specific environment with custom-made stuff.

Use direnv for this (make sure it is installed with your package manager).

#+begin_src emacs-lisp
(defun my/load-direnv-env ()
  "Load environment using direnv with fish for the current buffer's project."
  (let* ((project-dir (or (when-let ((proj (project-current)))
         ...
         (env-output
          (with-temp-buffer
            (when (zerop
                   (call-process
                    "/usr/bin/direnv" nil t nil
                    "exec" project-dir "fish" "-c" "env"))
              (buffer-string)))))
    (when env-output
        ...
        (when (string-match "\\`\\([^=]+\\)=\\(.*\\)\\'" line)
          (let ((var (match-string 1 line))
            ...
            (when (string= var "PATH")
              (setq exec-path
                    (append (parse-colon-path val)
                            (list exec-directory))))))))
    (message "[env] Loaded env for %s" project-dir)))

(defun my/setup-direnv-and-lsp ()
  "Load direnv env and start lsp-deferred if any LSP-relevant executable is found."
  (my/load-direnv-env)
  (cond
   ;; Elixir
   ((executable-find "elixir")
    (message "[env+lsp] Found elixir, starting LSP.")
    (lsp-deferred))
   ;; Node/JS/TS
   ((executable-find "node")
    (message "[env+lsp] Found node, starting LSP.")
    (lsp-deferred))
   ;; Python (example)
   ((executable-find "python")
    (message "[env+lsp] Found python, starting LSP.")
    (lsp-deferred))
   ;; Go
   ((executable-find "go")
    (message "[env+lsp] Found Go, starting LSP.")
    (lsp-deferred))
   ;; Default fallback
   (t
    (message "[env+lsp] No known language runtime found in PATH."))))

(dolist (hook '(elixir-ts-mode-hook
                typescript-ts-mode-hook
                tsx-ts-mode-hook
                js-ts-mode-hook
                python-ts-mode-hook
                go-ts-mode-hook))
  (add-hook hook #'my/setup-direnv-and-lsp))
#+end_src
